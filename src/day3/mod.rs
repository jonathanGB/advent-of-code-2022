...
fn build_priority_map() -> HashMap<char, u32> {
  ('a'..='z').chain('A'..='Z').enumerate().map(|(priority, item)| (item, (priority+1) as u32)).collect()
}
...
impl Solver for Day3Solver {
  fn solve_part1() {
    ...
    for line in file.lines() {
      ...
      let mistaken_item: &char = first_compartment.intersection(&second_compartment).next().unwrap();
  ...
  fn solve_part2() {
    ...
    while lines.peek().is_some() {
      ...
      let first_and_second_rupsack_common: HashSet<char> = first_rupsack.intersection(&second_rupsack).map(|item| *item).collect();
      let badge = first_and_second_rupsack_common.intersection(&third_rupsack).next().unwrap();
